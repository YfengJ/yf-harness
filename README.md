...
- PySide6 + Qt Quick 原生桌面 App：Composer 内直接切换 Agent/Plan、模型、持久 Goal 与上下文概览。
...
Composer 底栏可以直接切换 Agent/Plan、选择当前 Provider 的模型、维护会话 Goal，并查看真实上下文占用。
也可以在输入框使用 `/goal <目标>`、`/goal done` 和 `/goal clear`。活动 Goal 会跟随会话保存并注入后续
运行，但不会自动执行任务、扩大工具权限或跳过审批。
