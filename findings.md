...
## 阶段 22：全项目审阅与公开发布（2026-09-05）

- 完整缺陷表、测试证据和残余风险集中在 `docs/AUDIT_2026-09-05.md`。以实际复现修复为准，不把宽范围审阅等同于零缺陷保证。
- 数据库迁移必须在持有写事务后读取版本，不能使用会隐式提交的 `executescript`；恢复仅处理中断拥有者，而不是所有 RUNNING 记录。
- 审批与取消不可留下 `asyncio.to_thread(Event.wait)` 的永久线程；桌面采用异步事件和视图代次隔离结果。
- 工具输出在读取时限量，不是读取全部后截断；MCP 结构化返回也受同一预算约束。
- 后续人工编辑必须在审批和撤销路径都受保护；TUI 与持久化桌面撤销是两条入口，不能仅验证其中一条。
- 公开授权以用户最新指令为准，先扫描再发布；安全政策保留 NLTK 可选依赖未修复公告，CI 仅接受精确、限时例外。
