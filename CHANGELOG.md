...
- SQLite 迁移串行化、事务回滚；v6 记录运行进程，避免第二个 App/CLI 启动时误中断仍在执行的任务。
- 上下文准备异常正确结束运行；压缩保留工具调用与结果分组，TUI 恢复/保存摘要且不把自动上下文存成原始用户消息。
- Provider 遵守非流式模型能力，缺少 usage 时明确估算；SSE 意外 EOF 不再把不完整工具调用当成成功。
- Shell 双通道有界读取、UTF-8 增量解码、超时保留输出，POSIX 清理持有管道的后代进程；MCP 限制分页与结构化输出并清理进程组。
- Patch 正确处理零长度插入和无末尾换行；恢复文件保留执行权限，临时写入使用随机文件名。
- TUI 撤销拒绝覆盖后续编辑并保留失败记录；`exist_ok` 使用已有目录时不生成可能删除它的撤销记录。
- 桌面异步结果防串会话/项目，审批超时不再挂住后台线程；恢复工作区时加载项目配置，发送失败不清除待发送附件。

### Changed

- 桌面消息支持选中复制；用户文本不解释为 Markdown；手动向上阅读时暂停跟随输出，并提供回到最新消息入口。
- 键盘按钮、Composer 操作与文本框补齐可访问名称；保留输入框内模型、模式、Goal 和上下文控制。
- 新增公开安全政策、全历史 Gitleaks 和全 extras 依赖审计 CI；限时、精确记录可选 NLTK 未修复公告。

### Security

- 日志在格式化参数及异常后统一脱敏；关闭被替换的日志处理器。
- 自动索引跳过常见密钥/数据库/环境文件并剪枝依赖目录；明确远程模型会接收被选入上下文的代码。
- 文件写审批后复核目标快照，拒绝覆盖审批期间的人工变更；只读 Git 禁用外部 Diff/textconv/fsmonitor。
- GitHub push 校验全部推送 URL 均属于已确认仓库，拒绝额外或不同目的地。
- 升级 pytest 修复已知公告；NLTK 可选依赖例外与到期策略见 `SECURITY.md`，不冒充漏洞已修复。

## [0.12.0] - 2026-09-02

### Added

- Composer 内联模型、Agent/Plan 模式、持久 Goal 与真实上下文概览，支持键盘快捷切换。
- “工具与连接”中心：可安全启用 Brave Search MCP 或自定义 stdio MCP，并测试实际工具发现。
- 项目 Skills 管理器：查看、创建、导入本地文件夹及从 GitHub 仓库安装，安装后立即校验和调用。
- 当前仓库 GitHub 工作区：查看私密性、分支、PR、Issue、Actions，并执行受限的同步、建分支、创建和评论操作。
- 十个 GitHub 工具；与原有本地工具共同进入模型工具注册表、Schema 校验与审批链。

### Changed

- 桌面审批完整展示路径、命令、参数、Diff 与网络访问，并区分单次允许、会话允许、拒绝和取消运行。
- MCP 支持逐工具本地风险覆盖；Brave 只暴露网页、新闻和 LLM 上下文搜索，网络调用仍需审批。
- App 管理的非密钥连接配置采用原子写入；Brave API Key 使用系统钥匙串，环境变量优先且不写入仓库。

### Security

- GitHub 只绑定当前 workspace 的 `github.com` origin，复用已有 `gh` 登录；不提供强推、删除、合并或仓库设置能力。
- GitHub 写工具始终审批，远端内容标记为不可信；Skills 拒绝符号链接、越界、超限和覆盖已有目录，附带脚本绝不自动执行。
- GitHub CLI 使用用户级 HOME/GH_CONFIG_DIR，避免认证状态或设备标识落入项目目录。

## [0.11.0] - 2026-09-01

### Added

- 桌面 Composer 的统一 `+` 附件入口，同时支持图片和项目内 UTF-8 文本/代码文件。
- 普通文件附件使用 200 KB 上限、workspace 隔离、二进制/编码检查和发送前 SHA-256 复核，并作为真实 ContextBuilder 来源进入模型上下文。

### Changed

- 空会话移除三条装饰性任务说明，只保留一句简短引导。
- Composer 移除常驻 Goal、上下文、模型、模式、命令和额度信息，只保留附件、输入与发送。
- 左下角新增“设置与用量”入口，Provider、模型、工作流、模式、权限、上下文和本地额度统计迁入独立抽屉。
